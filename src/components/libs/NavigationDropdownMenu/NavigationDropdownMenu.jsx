...
export default function NavigationDropdownMenu() {
    ...
    return (
        <div className="nav-dropdown-root" ref={menuRef}>
            <div className="avatar relative ml-7 nav-profile-container" onClick={() => setIsOpen((prev) => !prev)}>
                <div className="w-14 max-[480px]:w-10 rounded-full border-2 border-white relative nav-profile-img">
                    <img
                        ...
                        alt="Profile"
                        onError={e => { e.target.onerror = null; e.target.src = ProfileHolder; }}
                    />
                </div>
                ...
                <div className="nav-dropdown-menu">
                    {subMenu === null ? (
                        <ul className="nav-dropdown-list">
                            <li className="avatar nav-dropdown-avatar-item" onClick={e => { e.stopPropagation(); handleNavigate(Pathname('PROFILE')); }}>
                            ...
                            <li className='nav-sub-expand' onClick={e => { e.stopPropagation(); setSubMenu('chat'); }}>
                                <div className='nav-dropdown-sub-container'>
                                    <img className='nav-dropdown-item-icon' src={ChatIcon} alt="Chats" />
                                    <span className='oxanium-semibold'>Chats</span>
                                </div>
                                <img className='nav-dropdown-item-subIcon' src={SubArrow} alt="Submenu" />
                            </li>
                            <li className='nav-sub-expand' onClick={e => { e.stopPropagation(); setSubMenu('notification'); }}>
                                <div className='nav-dropdown-sub-container'>
                                    <img className='nav-dropdown-item-icon' src={BellIcon} alt="Notifications" />
                                    <span className='oxanium-semibold'>Notifications</span>
                                </div>
                                <img className='nav-dropdown-item-subIcon' src={SubArrow} alt="Submenu" />
                            </li>
                            <div className="nav-dropdown-divider" />
                            <li onClick={e => { e.stopPropagation(); handleNavigate(Pathname('SETTING_PAGE')); }}>
                                <img className='nav-dropdown-item-icon' src={SettingIcon} alt="Settings" />
                                <span className='oxanium-semibold'>Settings</span>
                            </li>
                            <li onClick={e => { e.stopPropagation(); handleLogout(); }}>
                                <img className='nav-dropdown-item-icon' src={LogoutIcon} alt="Logout" />
                                <span className='oxanium-semibold'>Logout</span>
                            </li>
                        </ul>
                    ) : subMenu === 'chat' ? (
                        <div className={`nav-submenu-panel${submenuExiting ? ' exiting' : ''}`} onClick={e => e.stopPropagation()}>
                            <div className="nav-submenu-header oxanium-semibold">
                                <div onClick={handleCloseSubMenu} className='nav-dropdown-close-subIcon'>
